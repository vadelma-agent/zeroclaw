...
preserve their original meaning through that parse. `context_window` controls
ZeroClaw's best-effort local history budgeting; `0` means that no provider-wide
context assumption is made. It is deliberately not sent to Hailo-Ollama.
Call-level native thinking requests are rejected before any backend request.
Responses must be a completed non-streaming response
