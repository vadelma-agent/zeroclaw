...
impl HailoOllamaModelProvider {
    ...
    pub(crate) fn with_auth_headers(
        mut self,
        api_key: Option<&str>,
        ...
        if let Some(api_key) = api_key.map(str::trim).filter(|value| !value.is_empty()) {
            let mut value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|_| {
                anyhow::Error::msg("Hailo-Ollama API key is not a valid HTTP header value")
            })?;
        ...
        for (name, value) in extra_headers {
            ...
            let mut header_value = HeaderValue::from_str(value).map_err(|_| {
                anyhow::Error::msg(format!(
                    "Hailo-Ollama extra header `{name}` has an invalid value"
                ))
            })?;
...
impl HailoOllamaModelProvider {
    ...
    fn normalize_messages(&self, messages: Vec<Message>) -> anyhow::Result<Vec<Message>> {
        ...
        if first_user == candidates.len() && !candidates.is_empty() {
            ::zeroclaw_log::record!(
                WARN,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Reject)
                    .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({
                        "error_key": "hailo_history_missing_user_anchor",
                        "model_provider": self.alias,
                        "orphaned_messages": candidates.len(),
                    })),
                "Hailo-Ollama rejected history without a user anchor"
            );
            return Err(anyhow::Error::new(NonRetryableProviderError::new(
                "Hailo-Ollama history does not contain a user-anchored turn",
            )));
        }
        if first_user > 0 {
            ::zeroclaw_log::record!(
        ...
        let next_user_turn = |candidates: &[MessageCandidate]| {
            candidates
                .iter()
                .enumerate()
                .skip(1)
        ...
        let projected_chars = |candidates: &[MessageCandidate]| -> anyhow::Result<usize> {
            ...
            for (index, candidate) in candidates.iter().enumerate() {
                ...
                let content_chars = if index == 0 && !system.is_empty() {
                    self.fold_system(&system, &candidate.content)?
                        .chars()
                        .count()
                } else {
            ...
            Ok(total_chars.saturating_add(current_message_chars))
        };
        let retained_chars = loop {
            match projected_chars(&candidates) {
                Ok(chars) if chars <= history_char_budget => break chars,
                result => {
        ...
        if retained_chars > history_char_budget && !candidates.is_empty() {
            ::zeroclaw_log::record!(
                WARN,
                ::zeroclaw_log::Event::new(module_path!(), ::zeroclaw_log::Action::Reject)
                    .with_outcome(::zeroclaw_log::EventOutcome::Failure)
                    .with_attrs(::serde_json::json!({
                        "error_key": "hailo_latest_turn_exceeds_context_budget",
                        "model_provider": self.alias,
                        "retained_messages": candidates.len(),
                        "retained_chars": retained_chars,
                        "history_char_budget": history_char_budget,
                    })),
                "Hailo-Ollama rejected a current prompt that exceeds its local context budget"
            );
            return Err(anyhow::Error::new(NonRetryableProviderError::new(format!(
                "Hailo-Ollama current prompt exceeds the local context budget \
                     ({retained_chars} chars > {history_char_budget}); increase context_window \
    ...
    async fn send_request(
        ...
        let result = worker
            .await
            .map_err(|error| self.request_worker_failure(&gate, &error))?;
...
impl ModelProvider for HailoOllamaModelProvider {
    ...
    async fn chat_with_tools(
        ...
        if !tools.is_empty() {
            return Err(anyhow::Error::new(NonRetryableProviderError::new(
                "Hailo-Ollama does not support native tool calling",
            )));
        }
        let response = self.chat_messages(messages, model, temperature).await?;
...
mod tests {
    ...
    #[tokio::test]
    async fn closed_generation_semaphore_is_non_retryable() {
        let provider = HailoOllamaModelProvider::new(
            "closed-gate",
            Some("http://127.0.0.1:65531/closed-gate-test"),
            90,
            5,
            OllamaTuning::default(),
        )
        .expect("valid isolated Hailo URL");
        provider.generation_gate.semaphore.close();

        let error = provider
            .send_request(Vec::new(), "qwen3:1.7b", None)
            .await
            .expect_err("a closed generation semaphore must reject before transport");

        assert!(
            error
                .chain()
                .any(|source| source.is::<NonRetryableProviderError>()),
            "a permanently closed gate must suppress generic retries: {error:#}"
        );
    }

    #[tokio::test]
    async fn worker_failure_is_non_retryable_and_quarantines_gate() {
        let provider = HailoOllamaModelProvider::new(
            "worker-failure",
            Some("http://127.0.0.1:65532/worker-failure-test"),
            90,
            5,
            OllamaTuning::default(),
        )
        .expect("valid isolated Hailo URL");
        let worker = zeroclaw_spawn::spawn!(async move {
            panic!("synthetic request worker panic");
        });
        let join_error = worker.await.expect_err("synthetic worker must panic");

        let error = provider.request_worker_failure(&provider.generation_gate, &join_error);

        assert!(
            error
                .chain()
                .any(|source| source.is::<NonRetryableProviderError>()),
            "a failed request worker must suppress generic retries: {error:#}"
        );
        assert_eq!(
            provider.generation_gate.quarantine_reason.get(),
            Some(&QuarantineReason::WorkerTerminated)
        );
    }
